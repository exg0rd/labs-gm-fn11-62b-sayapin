...
let Coefficients = [0.2969, 0.126, 0.3516, 0.2843, 0.1015];
...
function main() {
    ...
    guiSurfaceParams.add(Data.controlsParameters, 'generateCoefficients',).onChange(function (e) { generateCoefficients(); Data.generateBoundaryCurves(Data.controlsParameters.slices), Data.calculateAndDraw(); });
...
const Curves = {
    yt: function (x, t) {
        ...
        return 5 * t * (Coefficients[0] * Math.sqrt(x) - Coefficients[1] * x - Coefficients[2] * (x ** 2) + Coefficients[3] * (x ** 3) - Coefficients[4] * (x ** 4));
    },
    yt_x: function (x, t) {
        return 5 * t * (Coefficients[0] * 0.5 * (x ** (-0.5)) - Coefficients[1] - Coefficients[2] * 2 * x + Coefficients[3] * 3 * (x ** 2) - Coefficients[4] * 4 * (x ** 3));
    },
    c1: function (u, pt) {
        let t = 0.15;
        if (Data.controlsParameters.generateCoefficients === true) {
            t = Coefficients[5];
        }
